...
echo "-------------------"
echo ""
echo $TOKEN_DOCKER_HUB | docker login -v docker.io -u agus3rdyoga --password-stdin 
}
...
echo "--------------------------"
echo ""
docker push $(docker image ls | grep -E 'item-app|v1' | awk '{print $3}'|tail -1) docker://docker.io/agus3rdyoga/item-app:v1
}
